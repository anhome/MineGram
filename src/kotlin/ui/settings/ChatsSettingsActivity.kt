...
import desu.mintgram.ui.settings.speech.SpeechRecognitionLanguageActivity
...
class ChatsSettingsActivity : SettingsPageActivity() {
    ...
    override fun fillItems(items: ArrayList<UItem>, adapter: UniversalAdapter) {
        ...
        items.add(UItem.asHeader(LocaleController.getString(R.string.InuVoiceRecognition)))
        items.add(
            UItem.asButton(
                BUTTON_SPEECH_RECOGNITION_LANGUAGE,
                LocaleController.getString(R.string.InuRecognitionLanguage),
                speechRecognitionLanguageLabel(),
            )
        )
        if (InuConfig.SPEECH_RECOGNITION_LANGUAGE.value != "none" && AiConfigHelper.canUseAi()) {
            items.add(
                mkTwoLineCheckItem(
                    TOGGLE_SPEECH_RECOGNITION_AI_POSTPROCESSING,
                    R.string.InuPostProcessingWithAi,
                    R.string.InuPostProcessingWithAiInfo,
                    { InuConfig.SPEECH_RECOGNITION_AI_POSTPROCESSING.value },
                )
            )
        }
        if (VoskModelStorage.listDownloaded().isNotEmpty()) {
            items.add(
                UItem.asButton(BUTTON_DELETE_SPEECH_MODEL, R.drawable.msg_delete, LocaleController.getString(R.string.InuDeleteRecognitionModel)).red()
            )
        }
    ...
    override fun onClick(item: UItem, view: View, position: Int, x: Float, y: Float) {
        ...
        when (item.id) {
            ...
            BUTTON_SPEECH_RECOGNITION_LANGUAGE -> presentFragment(SpeechRecognitionLanguageActivity())
            TOGGLE_SPEECH_RECOGNITION_AI_POSTPROCESSING ->
                (view as? NotificationsCheckCell)?.isChecked = InuConfig.SPEECH_RECOGNITION_AI_POSTPROCESSING.toggle()
            BUTTON_DELETE_SPEECH_MODEL -> handleDeleteSpeechModelClick(view)
        }
    }

    private fun handleDeleteSpeechModelClick(anchor: View) {
        val downloaded = VoskModelStorage.listDownloaded()
        if (downloaded.isEmpty()) return
        if (downloaded.size == 1) {
            confirmDeleteSpeechModel(downloaded[0].language)
            return
        }
        val options = ItemOptions.makeOptions(this, anchor)
        downloaded.forEach { model ->
            options.add(R.drawable.msg_delete, SpeechModels.displayName(model.language), true) {
                confirmDeleteSpeechModel(model.language)
            }
        }
        options.show()
    }

    private fun confirmDeleteSpeechModel(language: String) {
        val builder = AlertDialog.Builder(context)
        builder.setTitle(LocaleController.getString(R.string.InuDeleteRecognitionModel))
        builder.setMessage(
            LocaleController.formatString(R.string.InuDeleteRecognitionModelInfo, SpeechModels.displayName(language))
        )
        builder.setNegativeButton(LocaleController.getString(R.string.Cancel), null)
        builder.setPositiveButton(LocaleController.getString(R.string.Delete)) { _, _ ->
            VoskModelStorage.delete(language)
            if (InuConfig.SPEECH_RECOGNITION_LANGUAGE.value == language) {
                InuConfig.SPEECH_RECOGNITION_LANGUAGE.value = "none"
            }
            listView.adapter.update(true)
            BulletinFactory.of(this).createSuccessBulletin(LocaleController.getString(R.string.InuRecognitionModelDeleted)).show()
        }
        showDialog(builder.create())
    }

    private fun speechRecognitionLanguageLabel(): String {
        val language = InuConfig.SPEECH_RECOGNITION_LANGUAGE.value
        return if (language == "none") LocaleController.getString(R.string.InuRecognitionLanguageOff)
        else SpeechModels.displayName(language)
    }
    ...
    companion object {
        ...
        private val BUTTON_SPEECH_RECOGNITION_LANGUAGE = InuUtils.generateId()
        private val TOGGLE_SPEECH_RECOGNITION_AI_POSTPROCESSING = InuUtils.generateId()
        private val BUTTON_DELETE_SPEECH_MODEL = InuUtils.generateId()
        ...
        val PAGE = SearchRegistry.Page(
            ...
            entries = listOf(
                ...
                SearchRegistry.Entry("recognition-language", R.string.InuRecognitionLanguage, BUTTON_SPEECH_RECOGNITION_LANGUAGE),
                SearchRegistry.Entry(
                    "speech-recognition-ai-postprocessing",
                    R.string.InuPostProcessingWithAi,
                    TOGGLE_SPEECH_RECOGNITION_AI_POSTPROCESSING,
                ),
