...
object RoundRecorderHelper {
    ...
    @JvmStatic
    fun tFromZoom(zoom: Float, min: Float, max: Float): Float {
    ...
    @JvmStatic
    @JvmOverloads
    fun attachZoomSlider(
        parent: FrameLayout,
        onZoom: Utilities.Callback<Float>,
        onPreset: Utilities.Callback<Float>? = null,
    ): ZoomControlView? {
        ...
        view.setDelegate(object : ZoomControlView.ZoomControlViewDelegate {
            override fun didSetZoom(zoom: Float) = onZoom.run(zoom)
            override fun didTapPreset(targetZoom: Float) {
                onPreset?.run(targetZoom)
            }
        })
        parent.addView(
            view,
            LayoutHelper.createFrame(
                // taller than the bare 50dp track (stock's own ChatAttachAlertPhotoLayout zoom
                // bar stays at 50dp) to leave room for the preset pill row drawn above it
                LayoutHelper.MATCH_PARENT, 74f,
