...
import desu.mintgram.ui.EditHistoryActivity
import desu.mintgram.ui.MessageDetailsActivity
import desu.mintgram.ui.ai.AskAiBottomSheet
...
object ChatHelper {
    ...
    fun finalizeMessageMenu(
        ...
        if (isMenuItemEnabled(MessageMenuConfig.Item.ASK_AI) && AiConfigHelper.canUseAi() &&
            !selectedObject.messageOwner.message.isNullOrEmpty()
        ) {
            items.add(LocaleController.getString(R.string.InuAiAskQuestion))
            options.add(OPTION_ASK_AI)
            icons.add(R.drawable.magic_stick_solar)
        }
        ...
        if (selectedObject.messageOwner.edit_date != 0 &&
            isMenuItemEnabled(MessageMenuConfig.Item.EDIT_HISTORY) &&
            MessageHistoryHelper.hasEditHistory(activity.currentAccount, dialogId, selectedObject.id)
        ) {
    ...
    fun processMenuOption(
        ...
        when (option) {
            ...
            OPTION_EDIT_HISTORY -> {
                activity.presentFragment(EditHistoryActivity(activity.dialogId, selectedObject.id))
            }
            ...
            OPTION_ASK_AI -> {
                val ctx = activity.parentActivity ?: return true
                val prompt = selectedObject.messageOwner?.message.orEmpty()
                activity.showDialog(AskAiBottomSheet(ctx, activity, prompt) { response -> insertAiResponse(activity, response) })
            }
    ...
    private fun insertAiResponse(activity: ChatActivity, response: String) {
        val enterView = activity.chatActivityEnterView ?: return
        val text = if (InuConfig.AI_INSERT_AS_QUOTE.value) {
