...
import desu.mintgram.helpers.chat.ChatActionsHelper
import desu.mintgram.helpers.chat.ChatHelper
...
class ChatMenuConfig(key: String) : MenuOrderConfig<ChatMenuConfig.Item>(key, Item.entries, OFF_BY_DEFAULT) {
    enum class Item(
        ...
        DELETED_MESSAGES("deleted_messages", listOf(ChatActionsHelper.ACTION_DELETED_MESSAGES), R.string.InuDeletedMessagesTitle, R.drawable.msg_delete);
...
class MessageMenuConfig(key: String) : MenuOrderConfig<MessageMenuConfig.Item>(key, Item.entries, OFF_BY_DEFAULT) {
    enum class Item(
        ...
        ASK_AI("ask_ai", listOf(ChatHelper.OPTION_ASK_AI), R.string.InuAiAskQuestion, R.drawable.magic_stick_solar),
