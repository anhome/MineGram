...
import desu.mintgram.helpers.maps.MapLibreMapsProvider

class ApplicationLoaderImpl : BaseApplicationLoaderImpl() {
    override fun onCreate() {
        super.onCreate()
        desu.mintgram.PluginsBridge.instance = desu.mintgram.plugins.PluginsController.getInstance()
        desu.mintgram.PluginsBridge.pluginsActivityFactory = { desu.mintgram.plugins.ui.PluginsActivity() }
        desu.mintgram.PluginsBridge.installPluginFromUri = { fragment, uri ->
            org.telegram.messenger.Utilities.globalQueue.postRunnable {
                val ctx = fragment.parentActivity
                val dest = if (ctx != null) desu.mintgram.plugins.ui.PluginFileUtils.copyToCache(ctx, uri) else null
                org.telegram.messenger.AndroidUtilities.runOnUIThread {
                    if (dest == null) {
                        org.telegram.ui.Components.BulletinFactory.of(fragment)
                            .createErrorBulletin(org.telegram.messenger.LocaleController.getString(org.telegram.messenger.R.string.InuPluginInstallCopyError))
                            .show()
                    } else {
                        desu.mintgram.plugins.PluginsController.getInstance().showInstallDialog(fragment, dest.absolutePath, true)
                    }
                }
            }
        }
        // the compiled Python SDK has com.exteragram.messenger.ExteraConfig hardcoded - see that
        // shim class for why. Keep it in sync with our own setting before the engine reads it.
        com.exteragram.messenger.ExteraConfig.pluginsSafeMode = desu.mintgram.plugins.PluginsConfig.pluginsSafeMode
        desu.mintgram.plugins.PluginsController.getInstance().init()
    }
