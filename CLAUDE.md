# Mintgram Agent Guide

Mintgram is a **patchset**, not a fork. `worktree/` is a stock Telegram checkout with
...
7. **No renames in stock. No removing stock imports** (except `desu.mintgram.*`).
...
public void doSomething() {
    if (desu.mintgram.InuConfig.MY_TOGGLE.getValue()) {
...
Conventions: expose the minimum, promote `private` → `public` over duplicating data, `inu_` prefix on new fields, entry point is always a call to `desu.mintgram.helpers.XxxHelper.*` — never inline logic.
...
| `onResume(LaunchActivity)` | `LaunchActivity.onResume` | monet refresh, crash sheet, first-run onboarding sheet |
...
- Pref key = snake_case of the field name; default is the second arg. SharedPreferences name: `mintgram`. Loaded once from `InuHooks.init`.
...
- Extend `desu.mintgram.ui.settings.SettingsPageActivity` (wraps `UniversalFragment` with edge-to-edge + insets + `showRestartBulletin()`). Register pages in `InuSettingsActivity`.
...
- `desu.mintgram.SearchRegistry` wires fork pages into stock settings search (`ProfileActivity.SearchAdapter`) and routes `tg://settings/inu/<slug>` deeplinks.
